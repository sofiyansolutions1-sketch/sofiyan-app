...
// I know that the error is because a </div> is missing its matching <div>
// Or the regex replacement ate the opening <div> and left the closing </div>
// Let's replace the whole `partnerBookings.map` block.
...
content = content.replace(
  /\{partnerBookings\.map\(b => \([\s\S]*?\}\)\}\s*<\/div>\s*\)\}/,
  correctBlock
);
