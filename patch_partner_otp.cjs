  ...
  const renderOtpModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl animate-slideUp">
...
// 3. Add renderOtpModal to render
content = content.replace(
  /\{renderPaymentModal\(\)\}/,
  "{renderPaymentModal()}\n      {renderOtpModal()}"
);
...
const buttonBlockRegex = /\{\(b\.status === 'accepted' \|\| b\.status === 'Forwarded'\) && \([\s\S]*?Cancel[\s\S]*?Mark Completed[\s\S]*?<\/button>\s*<\/div>\s*\)\}/;
...
// 6. Update mapping to show Customer Details if in_progress
content = content.replace(
  /\{\(b\.status === 'accepted' \|\| b\.status === 'Forwarded'\) && \(\s*<div className="mb-4 p-3/g,
