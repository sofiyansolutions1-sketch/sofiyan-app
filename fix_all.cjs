...
// Fix duplicate Calendar import
content = content.replace(/Calendar, Info \} from 'lucide-react';/, "Info } from 'lucide-react';");

const profileModalStateCode = `
...
// Find `export const CustomerPanel: React.FC = () => {`
const panelIdx = lines.findIndex(l => l.includes('export const CustomerPanel: React.FC = () => {'));
if (panelIdx !== -1) {
    lines.splice(panelIdx + 1, 0, profileModalStateCode);
}

content = lines.join('\n');

// Also inject the floating buttons below the main div return
const floatingBtns = `
      {/* Sticky Header Actions */}
      <div className="fixed top-20 right-4 sm:right-8 z-[60] flex flex-col gap-3 pointer-events-none">
         ...
         <button onClick={() => { if(cart.length > 0) setBookingStep('form') }} className="pointer-events-auto bg-indigo-950 w-12 h-12 rounded-2xl flex items-center justify-center shadow-xl shadow-indigo-900/30 border border-indigo-800 text-white hover:scale-105 transition-all relative">
...
// Insert after `<div className="min-h-screen bg-gray-50 pb-24 sm:pb-12 font-sans selection:bg-indigo-200">`
content = content.replace(
  /<div className="min-h-screen bg-gray-50 pb-24 sm:pb-12 font-sans selection:bg-indigo-200">/,
  `<div className="min-h-screen bg-gray-50 pb-24 sm:pb-12 font-sans selection:bg-indigo-200">\n${floatingBtns}`
);
