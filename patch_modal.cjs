...
const oldModalTop = `  const renderRegistrationModal = (isMandatoryUpdate = false) => {
    return (
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4 animate-in fade-in duration-300">`;
...
const newModalTop = `  const renderCameraModal = () => {
  ...
  const renderRegistrationModal = (isMandatoryUpdate = false) => {
    return (
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4 animate-in fade-in duration-300">
        {renderCameraModal()}
`;
