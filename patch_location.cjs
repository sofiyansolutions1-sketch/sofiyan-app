...
const regex = /const handleTrackLocation = \(\) => \{[\s\S]*?setIsTrackingLocation\(false\);\n\s+\},\n\s+\{ enableHighAccuracy: true, timeout: 10000, maximumAge: 0 \}\n\s+\);\n\s+\};/;
...
const replacement = `const handleTrackLocation = () => {
    ...
    // Add a manual timeout because some browsers (Chromium on Windows) silently fail to call callbacks if OS location is disabled
    const fallbackTimeout = setTimeout(() => {
        setIsTrackingLocation(false);
        alert("Location detection timed out. Please ensure your device Location Services (GPS) are enabled in settings.");
    }, 15000);
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // Set form data with new values. ALWAYS update the locationLink!
        setFormData(prev => ({
          ...prev,
          lat,
          lng,
      ...
      (error) => {
        ...
        setIsTrackingLocation(false);
        alert("Unable to fetch exact location. Please ensure location permissions are granted and GPS is on.");
      },
